...
{
    class NetworkCell : Rule
    {
        public override void Apply(Prison prison)
        {
            NetworkCell.CreateOutboundThrottlePolicy(prison.User.Username, prison.User.Username, prison.Rules.NetworkOutboundRateLimitBitsPerSecond);

            if (prison.Rules.UrlPortAccess > 0)
            {
                NetworkCell.RemoveOutboundThrottlePolicy(PrisonUser.GlobalPrefix + PrisonUser.Separator + prison.Rules.UrlPortAccess.ToString());
                NetworkCell.CreateOutboundThrottlePolicy(PrisonUser.GlobalPrefix + PrisonUser.Separator + prison.Rules.UrlPortAccess.ToString(), prison.Rules.UrlPortAccess, prison.Rules.AppPortOutboundRateLimitBitsPerSecond);
            }
        }
        ...
        public override CellInstanceInfo[] List()
        {
            ...
            foreach (ManagementObject policy in GetThrottlePolicies())
            {
                if (!string.IsNullOrWhiteSpace(policy["Name"] as string) && policy["Name"].ToString().StartsWith(PrisonUser.GlobalPrefix + PrisonUser.Separator))
                {
                    ...
                    result.Add(new CellInstanceInfo()
                    {
